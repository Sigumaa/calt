...
## MVP完了後 拡張計画 v2
### 優先順
1. 安全（Safety Baseline）
2. CLI（Guided CLI）
3. 価値実感サンプル

### フェーズA Safety Baseline（計画）
- `session mode=dry_run` を追加し、実変更を伴うapply系操作を抑止する計画とする。
- high-risk stepに対して、`plan承認` と `実行直前確認` の二段確認を必須化する計画とする。
- Docker必須実行ガードを導入し、対象プロファイルではDocker経由以外の実行を拒否する計画とする。
- 安全プロファイル `strict` / `dev` を定義し、承認強度・許可操作・実行制約を切り替える計画とする。

### フェーズB Guided CLI（計画）
- `calt wizard run` を追加し、session作成から実行までを段階案内する計画とする。
- `calt explain` を追加し、判断理由・状態・次アクションを説明可能にする計画とする。
- 失敗時導線として、再承認/再計画/再実行の推奨手順をCLIで提示する計画とする。

### フェーズC 価値実感サンプル3本（計画）
- サンプル1: `dry_run` で完結するread-only調査フローを提供する計画とする。
- サンプル2: preview/apply二相を伴う安全な更新フローを提供する計画とする。
- サンプル3: 失敗停止から `needs_replan` を経る復旧フローを提供する計画とする。

### v2受け入れ基準（計画）
- `dry_run` 中は実変更が発生しないことをテストで確認可能であること。
- high-risk stepは二段確認が未完了なら実行不可であること。
- Docker必須実行ガードが `strict` プロファイルで有効に機能すること。
- `strict` / `dev` の差分がAPI/CLIの両面で一貫していること。
- `wizard run` と `explain` により失敗時導線まで到達可能であること。
- 価値実感サンプル3本がドキュメント化され、再現手順を持つこと。
