...
- subagent-manager-workflow: 複数サブエージェント開発をPM専任で再現運用する進行管理スキル。Plan確定後に `.codex` と `.github` の不足を判定し、必要最小差分でブートストラップしたうえで、実装・レビュー・テスト・コミットを担当分離して委譲する。並列化判定、委譲文テンプレ、失敗復旧、ユーザー相談ゲート、報告フォーマットを参照資料で固定し、別モデルでも同手順で運用する。単独実装依頼、サブエージェント非利用、計画未確定、雑談のみでは使わない。 (file: `/home/shiyui/.codex/skills/subagent-manager-workflow/SKILL.md`)
...
- 複数サブエージェントで実装、レビュー、テスト、コミットを担当分離し、PMを進捗管理専任で運用したいときに使う。
- Plan確定後に `.codex/PROJECT_PLAN.md` `.codex/PLAN_PROGRESS.md` `.codex/STARTUP_CHECKLIST.md` と `.github/workflows` の不足を判定して最小差分で補完したいときに使う。
- 依存しない作業を並列で進め、待ち合わせ条件と直列化理由を明示して管理したいときに使う。
- 並列化判定、委譲文テンプレ、失敗復旧、相談ゲート、報告フォーマットを固定ルールで再現したいときに使う。
...
- PMが実装、レビュー、テスト、コミットを直接行う前提の依頼では使わない。
- アシスタント単独で実装、レビュー、テスト、コミットまで実施する依頼では使わない。
- サブエージェントを使わない前提が明示されている依頼では使わない。
- Plan未確定で、まず要件定義や設計だけを行う依頼では使わない。
- `.codex` と `.github` の不足判定や補完を行わずに着手したい依頼では使わない。
